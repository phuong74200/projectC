...
int addProductsThread(gpointer top) {
    ...
    if (strcmp(response, "none") != 0) {
        ...
        strcpy(colorData, sindex);
        strcat(colorData, xmlp("color1", response));
        g_signal_connect(obj, "clicked", G_CALLBACK(colorPicker), colorData);

        obj = gtk_builder_get_object(product, "product_form_color2");
        gdk_color_parse(xmlp("color2", response), &color);
        gtk_widget_modify_bg(GTK_WIDGET(obj), GTK_STATE_NORMAL, &color);
        strcpy(colorData, sindex);
        strcat(colorData, xmlp("color2", response));
        g_signal_connect(obj, "clicked", G_CALLBACK(colorPicker), colorData);
        ...
        gtk_widget_modify_bg(GTK_WIDGET(obj), GTK_STATE_NORMAL, &color);
        strcpy(colorData, sindex);
        strcat(colorData, xmlp("color3", response));
        g_signal_connect(obj, "clicked", G_CALLBACK(colorPicker), colorData);
