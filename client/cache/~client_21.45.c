...
void entrychecking1(GtkEntry *entry, gpointer user_data) {
    ...
    const char *letter = gtk_entry_get_text(usernameEntry);

    printf("%c\n", letter[strlen(letter) - 1]);

    int isAlpha = g_ascii_isalpha(letter[strlen(letter) - 1]);
    int isDitgit = g_ascii_isdigit(letter[strlen(letter) - 1]);

    if (isAlpha == 0 && isDitgit == 0) {
        gtk_entry_set_max_length(entry, strlen(letter));
        gtk_entry_set_max_length(entry, strlen(letter) - 1);
    } else {
