...
void loginForm_guestLoginBtn() {
    window = gtk_builder_get_object(loginForm, "loginPage");
...
void loginScreenDisplay() {
    ...
    GObject *login = gtk_builder_get_object(loginForm, "loginForm");

    gtk_container_add(mainWindow, login);

    button = gtk_builder_get_object(loginForm, "guestLoginBtn");
    g_signal_connect(mainWindow, "clicked", G_CALLBACK(loginForm_guestLoginBtn), NULL);
}
