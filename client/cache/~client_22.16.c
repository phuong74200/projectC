...
int adm_getUserThread() {
    ...
    if (userPosition < 15) {
        ...
        GObject *testTop = gtk_grid_get_child_at(adm_user, 1, 10);
        GObject *value;
        value = gtk_scrolled_window_get_vadjustment(adm_user);
...
void admin_users_manage() {
    ...
    g_signal_connect(scollWindow, "edge-overshot", G_CALLBACK(admUserEdgeReach), "");
    ...
    gc(request);
    if (userPosition <= 1) {
        gdk_threads_add_timeout(50, adm_getUserThread, "");
    }
}
