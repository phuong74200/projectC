...
void addItemEvent(gpointer data) {
    ...
    if (addItemDelay == 1) {
        ...
        gdk_threads_add_timeout(5000, resetItemDelay, "");
        obj = gtk_builder_get_object(admin_users_manage_window, "addItemBtn");
        gtk_button_set_label(obj, "please wait 2.5s");

        int isValid = 0;
        char *content = "";
        obj = gtk_builder_get_object(admin_users_manage_window, "productPriceEntry");
        content = gtk_entry_get_text(obj);
        printf("%d len: ", strlen(content));
        for (int i = 0; i < strlen(content); i++) {
            int isDg = g_ascii_isdigit(content[i]);
            if (isDg == 0 || strlen(content) == 0) {
                isValid++;
                obj = gtk_builder_get_object(admin_users_manage_window, "saleoutDes");
                gtk_label_set_text(obj, "Set sale out value (Can't leave blank and must be an integer!)");
                break;
            } else {
                obj = gtk_builder_get_object(admin_users_manage_window, "saleoutDes");
                gtk_label_set_text(obj, "Set sale out value");
            }
        }
        ...
        for (int i = 0; i < strlen(content); i++) {
            ...
            if ((isDg == 0 && isAl == 0 && isSp == 0) || strlen(content) == 0) {
                isValid++;
