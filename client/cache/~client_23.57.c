...
void adm_getUserThread(gpointer data) {
    ...
    GtkWidget *usernameSlot = gtk_label_new();
    gtk_label_set_text(usernameSlot, xmlp("username", XMLBuffer));

    gtk_grid_attach(grid, usernameSlot, 0, 1, 1, 1);
...
void admin_users_manage() {
    ...
    gchar request[2000] = "<api>getUserList</api><index>0</index>";
