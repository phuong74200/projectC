...
void loginThread(gpointer request) {
    initSocket = socket(AF_INET, SOCK_STREAM, 0);
...
void threadParseXML(gpointer data) {
    char tag[200] = "<avail>";
...
void loginEvent(GtkWidget *widget, gpointer data) {
    ...
    printf("%s", request);
    g_thread_new("loginThread", loginThread, request);
    g_thread_new("x2", threadParseXML, "<avail>hello world</avail>");
}
