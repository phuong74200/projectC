    ...
    if (d) {
        while ((dir = readdir(d)) != NULL) {
            ...
            if (count > 0) {
                ...
                char request = "<api>getProduct</api><index>";
                strcat(request, quantity);
                strcat(request, "</index>");
                ...
                gtk_label_set_text(label, xmlp("name", response));
...
int addProductsThread(gpointer top) {
    ...
    if (strcmp(response, "none") != 0) {
        ...
        button = gtk_builder_get_object(product, "addToCartBtn");
        g_signal_connect(button, "clicked", G_CALLBACK(addItemToCartEvent), GINT_TO_POINTER(index));
