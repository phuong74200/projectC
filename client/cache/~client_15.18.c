...
int adm_getProductList() {
    if (getproducts_end == 1) {
        ...
        adm_product_grid = gtk_builder_get_object(admin_users_manage_window, "adm_user_grid");
        int top = userPosition + 1;
        ...
        if (productPosition < productListLength) {
            productPosition++;
            gdk_threads_add_timeout(10, adm_getProductList, "");
            getproducts_end = 1;
        }
    }
    return FALSE;
...
void admin_users_manage() {
    ...
    if (productPosition <= 1) {
        gdk_threads_add_timeout(250, adm_getProductList, "");
    }
}
