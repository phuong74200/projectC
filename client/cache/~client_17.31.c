...
void guestLogin(GObject *remover) {
    landingPageDisplay(remover);
}

void loginScreenDisplay() {
    window = gtk_builder_get_object(builder, "mainWindow");

    loginForm = gtk_builder_new_from_file("UI\\login.xml");

    loginForm = gtk_builder_get_object(loginForm, "loginForm");

    gtk_container_add(window, loginForm);

    button = gtk_builder_get_object(loginForm, "guestLoginBtn");
    g_signal_connect(loginForm, "clicked", G_CALLBACK(guestLogin(loginForm)), NULL);
}

GObject *mainWindow;

void landingPageDisplay(GObject *remover) {
    gtk_widget_destroy(remover);
    window = gtk_builder_get_object(builder, "landingPage");

    gtk_container_add(mainWindow, window);
    ...
    gtk_window_set_icon_from_file(GTK_WINDOW(mainWindow), "assets\\icon.png", NULL);
