...
addItemThread(gpointer data) {
    ...
    char name[100] = "";
    strcpy(name, gtk_entry_get_text(obj));
    ...
    itoa((int)color1->red, cstr, 10);
    strcat(c1str, cstr);
    strcat(c1str, ",");

    itoa((int)color1->green, cstr, 10);
    strcat(c1str, cstr);
    strcat(c1str, ",");

    itoa((int)color1->blue, cstr, 10);
    strcat(c1str, cstr);
...
void addItemEvent(gpointer data) {
    printf("addItem!");
    gdk_threads_add_timeout(50, addItemThread, "");
}
