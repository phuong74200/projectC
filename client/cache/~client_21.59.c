        ...
        if (strlen(content) == 0) {
            for (int i = 0; i < strlen(content); i++) {
                ...
                if (isDg == 0) {
                    ...
                    gtk_label_set_text(obj, "Set sale out value (must be an integer!)");
                    break;
                } else {
                    obj = gtk_builder_get_object(admin_users_manage_window, "saleoutDes");
            ...
            for (int i = 0; i < strlen(content); i++) {
                ...
                if (isDg == 0 && isAl == 0 && isSp == 0) {
                    ...
                    gtk_label_set_text(obj, "Name for item (not include special characters)");
                    break;
                } else {
                    obj = gtk_builder_get_object(admin_users_manage_window, "nameDes");
            ...
            obj = gtk_builder_get_object(admin_users_manage_window, "saleoutDes");
            gtk_label_set_text(obj, "Set sale out value (can't leave blank)");
            obj = gtk_builder_get_object(admin_users_manage_window, "nameDes");
