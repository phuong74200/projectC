...
void user_cart_event() {
    ...
    GObject *container = gtk_builder_get_object(user_cart, "user_cart_container");
    ...
    if (d) {
        while ((dir = readdir(d)) != NULL) {
            ...
            if (count > 0) {
                ...
                gtk_builder_add_from_file(user_cart, "UI\\cart.xml", &error);

                GObject *cart_grid = gtk_builder_get_object(user_cart, "user_cart");

                gtk_grid_attach(container, cart_grid, 0, 0, 1, 1);
