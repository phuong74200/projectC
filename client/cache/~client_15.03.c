...
void loginThread(gpointer requestBuffer) {
    ...
    char request[100] = "<api>login</api><username>";
    ...
    recv(initSocket, &response, 1000, 0);
...
void threadParseXML(gpointer *xmlStr) {
    char tag[200] = "<avail>";
    ...
    for (int pos = 0; pos < 2000; pos++) {
        int taginbuf = "";
        for (int tagdat = pos; tagdat < pos + len; tagdat++) {
            strcat(taginbuf, xmlStr[tagdat]);
        }
...
void loginEvent(GtkWidget *widget, gpointer data) {
    gdk_threads_add_idle(loginThread, "");
    gdk_threads_add_idle(threadParseXML, "<avail>hello world</avail>");
}
