...
GObject *registerContainer;
GObject *registerFrom;

GObject *loginContainer;
GObject *loginForm;
...
void backToLogin() {
    ...
    gtk_widget_destroy(registerFrom);
};
...
void registerNow(GtkWidget *widget, gpointer data) {
    ...
    registerContainer = gtk_builder_new_from_file("UI\\register.xml");

    registerFrom = gtk_builder_get_object(registerContainer, "registerForm");

    window = gtk_builder_get_object(builder, "mainWindow");

    gtk_container_add(window, registerFrom);

    gtk_window_set_title(GTK_WINDOW(window), "Register");

    gtk_widget_show_all(window);

    window = gtk_builder_get_object(registerContainer, "backToLogin");
...
void loginScreenDisplay() {
    ...
    loginContainer = gtk_builder_new_from_file("UI\\login.xml");

    loginForm = gtk_builder_get_object(loginContainer, "loginPage");

    window = gtk_builder_get_object(builder, "mainWindow");

    gtk_container_add(window, loginForm);
    ...
    window = gtk_builder_get_object(loginContainer, "guestLoginBtn");
    g_signal_connect(window, "clicked", G_CALLBACK(guestLogin), NULL);

    button = gtk_builder_get_object(loginContainer, "loginBtn");
    g_signal_connect(button, "clicked", G_CALLBACK(loginEvent), NULL);

    window = gtk_builder_get_object(loginContainer, "registerBtn");
