...
void rebackToLogin() {
    window = gtk_builder_get_object(builder, "registerPage");
    gtk_widget_destroy(window);
    window = loginPage;

    gtk_container_add(container, window);

    gtk_window_set_title(GTK_WINDOW(container), "Login");
    ...
    g_signal_connect(button, "clicked", G_CALLBACK(registerNow), NULL);

    gtk_widget_show_all(builder);
}
...
void registerNow() {
    ...
    gtk_window_set_title(GTK_WINDOW(container), "Register");

    button = gtk_builder_get_object(builder, "rebackToLogin");
    g_signal_connect(button, "clicked", G_CALLBACK(rebackToLogin), NULL);
}
...
void loginScreenDisplay() {
    ...
    loginPage = gtk_builder_get_object(builder, "loginPage");
